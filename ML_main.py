...
lcd.lcd_display_string(f"BabyCry Detect", 1)
...
def RecordAudio(dev,duration=5):
     ...
     print("Recording...")
     lcd.lcd_display_string(f"Recording...         ", 2)
     frames = []
...
# Record audio for the specified duration
     ...
     lcd.lcd_display_string(f"Finished recording.         ", 2)
     stream.stop_stream()
     stream.close()
     audio.terminate()
...
if __name__=="__main__":
    while True:
        but = GPIO.input(4)
        lcd.lcd_display_string(f"Push to Start      ", 2)
        if but == 0:
            GPIO.output(17, GPIO.HIGH)
            sleep(0.2)
            GPIO.output(17, GPIO.LOW)
            lcd.lcd_display_string(f"Started            ", 2)
            RecordAudio(1)
            lcd.lcd_display_string(f"Predicting..            ", 2)
            outP = Predict_Label(OUTPUT_FILENAME)
            print(outP)
            lcd.lcd_display_string(f"{outP}            ", 2)
            sleep(2)
